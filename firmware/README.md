...
制限されます。APIから開始した回転は安全のためデフォルトで3秒後に停止します。
ローカルNext.js操作画面は接続中、UDP 5000番へ250ms間隔、HTTP制御モードでは750ms間隔で安全信号を送信します。
...
ローカルNext.js操作画面はUDP 5000番で低遅延制御プロトコルv2の同じ即時命令を3回
送信します。接続時だけACK付きの安全な停止命令でUDP制御の受信を確認し、通常操作では
v2を使います。v2はACKを生成せず、HTTP応答も待ちません。確認通信をリアルタイム操作経路
から分離しているため、TCPソケットの状態で開始・停止が遅延しません。診断ツール向けの
従来プロトコルv1だけは適用結果のUDP ACKを返します。後から届いた古い複製は無視される
ため、素早く停止・再回転しても順序が逆転しません。
USBシリアルからの指令にはこのタイムアウトは適用されません。

W5500のソケット命令とPIO FIFO書き込みには有限の待ち時間を設けています。W5500がTCP切断や
UDP受信処理中に応答しなくなった場合は、モーターを停止してEthernetコントローラーだけを
自動リセットし、取得済みのネットワーク設定で復帰します。`/api/status` の
`socketCommandTimeouts` は起動後にこの復旧が発生した回数です。さらに、想定外の
ネットワークライブラリ停止に備えてPico本体の2秒ウォッチドッグも有効です。

W5500の物理LANリンクは1ms周期で監視し、ケーブル断などでリンクが切れた場合は
両モーターを即時停止します。スイッチ経由でホストだけが停止した場合は物理リンクが
維持されるため、安全信号が途絶えてから最大3秒で停止します。
