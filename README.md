...
- LAN経由の運転命令には0.5秒のウォッチドッグがあります。画面は接続中、HTTP状態確認とは独立したUDP安全信号を100ms周期で送り、信号が0.5秒途絶えるとPicoが両モーターを自動停止します。
